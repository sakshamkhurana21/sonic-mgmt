#! /usr/bin/env python3
'''
    These tests check the Vxlam ecmp with vnet-ping TSA/TSB functionality. Further details are
...
from tests.vxlan.vnet_monitor_utils import setup_vnet_ping_responder     # noqa: F401
...
class Test_VxLAN_VNET_PING_TSA():
    '''
    ...
    def create_vnet_route(self, encap_type):
        ...
        primary_nhg = end_point_list[0:2]
        ...
        #       "primary": "{priamry_end_point_list}",
        #       "monitoring": "custom",
        #     },
        #     "OP": "{}"
        #   }
        ...
        ecmp_utils.create_and_apply_priority_config(
            ...
            primary_nhg,
            "SET")
        return dest, end_point_list, primary_nhg
    ...
    def verfiy_all_vnet_ping_down(self):
        duthost = self.vxlan_test_setup['duthost']
        cmd = "redis-cli -n 6 KEYS 'VNET_MONITOR_TABLE|*'"
        result = duthost.shell(cmd)['stdout_lines']
        for key in result:
            cmd = "redis-cli -n 6 hget '{}' 'state'".format(key)
            res = duthost.shell(cmd)['stdout_lines']
            if res == "up":
                return False
        return True
    ...
    def test_tsa_case1(self, setUp, encap_type, setup_vnet_ping_responder):
        '''
            ...
            3) apply TSA.
            4) check TSC is maintenance.
            5) Verify vnet-ping sessions are down.
            6) apply TSB.
        ...
        dest, ep_list, primary = self.create_vnet_route(encap_type)
        downed_endpoints = []
        for ep in ep_list:
            if ep not in primary:
                downed_endpoints.append(ep)
        self.dump_self_info_and_run_ptf("test1", encap_type, True, downed_endpoints)

        self.apply_tsa()
        pytest_assert(self.in_maintenance())
        pytest_assert(self.verfiy_all_vnet_ping_down(), "Ping sessions are not down while device is in TSA")

        self.apply_tsb()
        pytest_assert(not self.in_maintenance())

        self.dump_self_info_and_run_ptf("test1b", encap_type, True, downed_endpoints)
    ...
    def test_tsa_case2(self, setUp, encap_type, setup_vnet_ping_responder):
        '''
            ...
            1) apply TSA.
            2) check TSC is maintenance.
            3) Verify vnet-ping sessions are down.
            4) Create Vnet route with 4 endpoints and vnet-ping monitors.
            5) apply TSB.
        ...
        self.dump_self_info_and_run_ptf("test2", encap_type, True, downed_endpoints)
    ...
    def test_tsa_case3(self, setUp, encap_type, setup_vnet_ping_responder):
        '''
            ...
            6) Create Vnet route with 4 endpoints and vnet-ping monitors.
            7) send packets to the route prefix dst. packets are received at all 4 endpoints.
            8) Delete route.
        '''
        self.vxlan_test_setup = setUp
        ...
        self.dump_self_info_and_run_ptf("test3", encap_type, True, downed_endpoints)
    ...
    def test_tsa_case4(self, setUp, encap_type, setup_vnet_ping_responder):
        '''
            ...
            4) apply TSA.
            5) check TSC is maintenance.
            6) Verify vnet-ping sessions are down.
        ...
        self.dump_self_info_and_run_ptf("test5", encap_type, True, downed_endpoints)

        self.apply_tsa()
        pytest_assert(self.in_maintenance())
        pytest_assert(self.verfiy_all_vnet_ping_down(), "Ping sessions are not down while device is in TSA")

        config_reload(duthost, wait=500, wait_for_bgp=True)
        ...
        self.dump_self_info_and_run_ptf("test5b", encap_type, True, downed_endpoints)
    ...
    def test_tsa_case5(self, setUp, encap_type, setup_vnet_ping_responder):
        '''
            ...
            7) Verify vnet-ping sessions are down.
        ...
        self.dump_self_info_and_run_ptf("test6", encap_type, True, downed_endpoints)
