...
function show_help_and_exit() {
    ...
    echo "************************************ IMPORTANT ****************************************"
    echo ""
    echo "This script is for setting up sonic-mgmt container for running tests."
    echo "Currently we have 2 flavor of sonic-mgmt docker images:"
    echo "  1. sonicdev-microsoft.azurecr.io:443/docker-sonic-mgmt:latest"
    echo "  2. sonicdev-microsoft.azurecr.io:443/docker-sonic-mgmt:py3only"
    echo ""
    echo "Image #1 has all packages installed for running tests with python2 and python3."
    echo "Image #2 has only python3 packages installed."
    echo ""
    echo "For sonic-mgmt test code, different python versions required for different branches."
    echo "Different sonic-mgmt docker images are used for different branches."
    echo ""
    echo "+------------------+----------------+--------------------------------------------------------+"
    echo "| Branch name      | Python version | Docker image                                           |"
    echo "+------------------+----------------+--------------------------------------------------------+"
    echo "| internal         | python3        | docker-sonic-mgmt:py3only or docker-sonic-mgmt:latest  |"
    echo "| internal-202311  | python3        | docker-sonic-mgmt:py3only or docker-sonic-mgmt:latest  |"
    echo "| internal-202305  | python3        | docker-sonic-mgmt:py3only or docker-sonic-mgmt:latest  |"
    echo "| internal-202205  | python2        | docker-sonic-mgmt:latest                               |"
    echo "| internal-202012  | python2        | docker-sonic-mgmt:latest                               |"
    echo "+------------------+----------------+--------------------------------------------------------+"
    echo ""
    echo "Behavior of this script is different in different branches. Please pay attention to the current branch."
    echo "* For internal-202305 and later branches, this script will use docker-sonic-mgmt:py3only image by default."
    echo "* For internal-202205 and older branches, this script will use docker-sonic-mgmt:latest image by default."
    echo ""
    echo "Because python2 is EOL, vulnerabilities in python2 packages are not fixed. So, we are moving to python3."
    echo "If you need to setup a sonic-mgmt container in your SONiC DEV VM, please only use docker-sonic-mgmt:py3only image."
    echo "Otherwise, we will receive security warnings from the security team."
    echo ""
    echo "If you need to run python2 tests in internal-202205 or internal-202012 branches, please try to setup sonic-mgmt"
    echo "container using docker-sonic-mgmt:latest image in lab server instead of SONiC DEV VM to avoid security warnings."
    echo
    echo "Other options:"
...
RUN sudo rm -rf /usr/local/lib/python3.8/dist-packages/tests
